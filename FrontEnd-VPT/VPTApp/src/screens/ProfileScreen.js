...
const ProfileScreen = ({ navigation }) => {
  ...
  const loadUserData = async () => {
    try {
      ...
      // Get user profile
      const { data: profileData, error: profileError } = await api.getUserProfile();
      ...
      if (profileData) {
        ...
        const { data: experienceData, error: experienceError } = await api.getUserExperienceLevel();
  ...
  const handleSubmitQuestionnaire = async (results) => {
    try {
      setLoading(true);
      const { error } = await api.saveQuestionnaireSession(results.formattedData);
      
      if (error) throw error;
      
      // Update local state with new experience level
      setExperienceLevel({
